  ...
  useEffect(() => {
    async function loadCounts() {
      ...
      const results = await Promise.all(
        SECTIONS.map((s) => supabase.from("teachers").select("id", { count: "exact", head: true }).eq("branch", s.key))
      );
      const newCounts = {};
      let total = 0;
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{loading ? "Loading..." : totalCount + " teachers across all sections"}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-8">
        {SECTIONS.map((s) => (
          <Link key={s.key} href={"/admin-dashboard/teachers/" + s.key.toLowerCase()} className="border border-slate-200 rounded-2xl p-6 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 block">
            ...
            <p className="text-sm text-brand-blue-strong font-medium mt-4">View teachers →</p>
          </Link>
        ))}
      </div>
    </div>
