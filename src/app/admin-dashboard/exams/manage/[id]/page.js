...
export default function ManageExam() {
  ...
  async function approveAndPublish() {
    setPublishing(true);
    const { error } = await supabase.from("exams").update({ submission_status: "published", status: "published" }).eq("id", id);
    setPublishing(false);
    if (!error) setExam((prev) => ({ ...prev, submission_status: "published", status: "published" }));
  }

  async function sendBackToDraft() {
    setPublishing(true);
    const { error } = await supabase.from("exams").update({ submission_status: "draft", status: "draft" }).eq("id", id);
    setPublishing(false);
    if (!error) setExam((prev) => ({ ...prev, submission_status: "draft", status: "draft" }));
  }
  ...
  return (
    <div>
      <Link href={"/admin-dashboard/exams/" + exam.branch.toLowerCase()} className="text-sm text-brand-blue-strong font-medium mb-2 inline-block hover:underline">← Back to {exam.branch} Exams</Link>
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          {exam.submission_status === "pending_review" && (
            <>
              <button onClick={sendBackToDraft} disabled={publishing} className="text-sm font-medium px-5 py-2.5 rounded-lg border border-slate-300 hover:bg-slate-50 disabled:opacity-50">Send Back</button>
              <button onClick={approveAndPublish} disabled={publishing || questions.length === 0} className="text-sm font-medium px-5 py-2.5 rounded-lg bg-brand-blue-strong text-white hover:opacity-90 disabled:opacity-50">{publishing ? "..." : "Approve & Publish"}</button>
            </>
          )}
          {exam.submission_status === "published" && (
            <button onClick={sendBackToDraft} disabled={publishing} className="text-sm font-medium px-5 py-2.5 rounded-lg border border-slate-300 hover:bg-slate-50 disabled:opacity-50">{publishing ? "..." : "Unpublish"}</button>
          )}
          {exam.submission_status === "draft" && (
            <span className="text-sm font-medium px-5 py-2.5 rounded-lg bg-slate-100 text-slate-500">Waiting on teacher</span>
          )}
        </div>
      </div>
      </div>
