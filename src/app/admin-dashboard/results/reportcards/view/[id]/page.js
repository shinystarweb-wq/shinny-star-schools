...
export default function ReportCardPage() {
  ...
  const [showSubjectPosition, setShowSubjectPosition] = useState(false);
  ...
  useEffect(() => {
    async function load() {
      ...
      for (const subj of (subjectData || []).map((s) => s.name)) {
        ...
        for (const term of TERMS) {
          ...
          for (const mate of classmates || []) {
            ...
            if (mateResult) {
              scoresForSubject.push({ id: mate.id, score: Number(mateResult.total_score) });
            }
          }
      ...
      const { data: positionSettings } = await supabase.from("class_settings").select("show_position, show_subject_position").eq("branch", studentData.branch).eq("class", studentData.class).eq("session", SESSION).maybeSingle();
      const positionOn = positionSettings?.show_position || false;
      setShowPosition(positionOn);
      setShowSubjectPosition(positionSettings?.show_subject_position || false);
    ...
    <div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-brand-blue text-slate-700">
              <tr>
                ...
                {showSubjectPosition && <th className="text-left px-4 py-2.5 font-semibold">Position</th>}
                ...
                subjects.map((subj) => {
                  ...
                  return (
                    <tr key={subj} className="border-t border-slate-200">
                      ...
                      {showSubjectPosition && <td className="px-4 py-2.5 text-slate-600">{subjPos ? ordinal(subjPos) : "-"}</td>}
      ...
      <style jsx global>{`
        @media print {
          ...
          #report-card table { font-size: 10px !important; }
          #report-card th, #report-card td { padding: 3px 6px !important; }
          #report-card .w-14 { width: 40px !important; height: 40px !important; }
          #report-card .mb-6 { margin-bottom: 8px !important; }
          #report-card .mt-10 { margin-top: 16px !important; }
          #report-card .pt-6 { padding-top: 8px !important; }
        }
      `}</style>
    </div>
  );
