...
export default function ReportCardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: settingsData } = await supabase
        ...
        .select("*")
        .eq("location", studentData.location)
        .single();
      ...
      for (const subj of (subjectData || []).map((s) => s.name)) {
        ...
        for (const term of TERMS) {
          ...
          for (const mate of classmates || []) {
            const { data: mateResult } = await supabase
              .from("results")
              .select("total_score")
              .eq("student_id", mate.id)
              .eq("subject", subj)
              .eq("term", term)
              .eq("session", SESSION)
              .eq("status", "published")
            ...
            if (mateResult) {
              scoresForSubject.push({
                id: mate.id,
                score: Number(mateResult.total_score),
              });
            }
          }
          ...
          const rank =
            scoresForSubject.findIndex((s) => s.id === id) + 1;
      ...
      const { data: positionSettings } = await supabase
        .from("class_settings")
      ...
      const positionOn =
        positionSettings?.show_position || false;
      ...
      if (positionOn) {
        ...
        for (const term of TERMS) {
          ...
          for (const mate of classmates || []) {
            const { data: mateResults } = await supabase
              .from("results")
              .select("total_score")
              .eq("student_id", mate.id)
              .eq("term", term)
            ...
            const sum = (mateResults || []).reduce(
              (s, r) => s + Number(r.total_score),
              0
            );
  ...
  function subjectRow(subject) {
    return TERMS.map((term) => {
      const r = (resultsByTerm[term] || []).find(
        (x) => x.subject === subject
      );
  ...
  function termTotal(term) {
    return (resultsByTerm[term] || []).reduce(
      (s, r) => s + Number(r.total_score),
      0
    );
  }
  ...
  function termAverage(term) {
    ...
    return (
      Math.round((termTotal(term) / rows.length) * 10) / 10
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 print:hidden">
        <Link
          href={
            "/admin-dashboard/results/reportcards/" +
            student.branch.toLowerCase() +
            "/" +
            encodeURIComponent(student.class)
          }
          className="text-sm text-brand-blue-strong font-medium hover:underline"
        >
          ← Back to Class
        </Link>
        ...
        <button
          ...
          className="bg-brand-blue-strong text-white text-sm font-medium px-5 py-2.5 rounded-lg hover:opacity-90"
        >
          🖨 Print Report Card
        </button>
      </div>
      ...
      <div
        className="border border-slate-200 rounded-2xl p-8 print:border-0 print:p-0 print:shadow-none max-w-4xl mx-auto bg-white"
        id="report-card"
      >
        <div className="text-center border-b-2 border-brand-blue-strong pb-4 mb-6">
          {schoolSettings?.logo_url ? (
            ...
            <div className="w-14 h-14 rounded-full bg-brand-blue flex items-center justify-center text-2xl mx-auto mb-2">
              🏫
            </div>
          )}
          ...
          <p className="text-xs text-slate-500 uppercase tracking-wider">
            {student.branch} Section — Student Report Card
          </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 text-sm">
          <div className="flex items-center gap-3 md:col-span-1 col-span-2">
            {student.photo_url ? (
              ...
              <div className="w-14 h-14 rounded-full bg-brand-blue flex items-center justify-center text-lg font-semibold text-brand-blue-strong ring-2 ring-brand-blue">
          ...
          <p>
            ...
            <span className="font-medium text-slate-800">
              {schoolSettings?.current_session || SESSION}
            </span>
          </p>
        </div>

        <h2 className="font-semibold text-slate-800 mb-3">
          {activeTerm} — Subject Scores
        </h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {subjects.length === 0 ? (
                ...
                subjects.map((subj) => {
                  const r = activeResults.find(
                    (x) => x.subject === subj
                  );

                  const cumulative = cumulativeFor(
                    subj,
                    activeTerm
                  );

                  const subjPos =
                    subjectPositions[subj]?.[activeTerm];
