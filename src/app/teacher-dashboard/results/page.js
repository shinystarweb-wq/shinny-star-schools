...
export default function TeacherResultsPage() {
  ...
  useEffect(() => {
    ...
    if (!stored) return;
    setBranch(JSON.parse(stored).branch);
  }, []);
  ...
  useEffect(() => { if (step === "class" && branch) pickClassStep(); }, [step, branch]);
  ...
  useEffect(() => { if (step === "subject" && branch) pickSubjectStep(); }, [step, branch]);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-6">{branch} Section — record CA1, CA2, and Exam scores.</p>

      {step !== "term" && (
        <div className="flex items-center gap-2 text-sm text-slate-500 mb-6">
          <button onClick={() => setStep("term")} className="hover:text-brand-blue-strong hover:underline">Term</button>
          {term && <span>/</span>}
          {term && (step === "class" ? <span className="text-slate-800 font-medium">{term}</span> : <button onClick={() => setStep("class")} className="hover:text-brand-blue-strong hover:underline">{term}</button>)}
          {className && step !== "class" && <span>/</span>}
      ...
      {step === "class" && (
        ...
        classOptions.length === 0 ? (
          <div className="border border-dashed border-slate-300 rounded-xl py-16 text-center"><p className="text-slate-500 text-sm">No classes found.</p></div>
        ) : (
      ...
      {step === "subject" && (
        ...
        subjectOptions.length === 0 ? (
          <div className="border border-dashed border-slate-300 rounded-xl py-16 text-center"><p className="text-slate-500 text-sm">No subjects configured yet.</p></div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
