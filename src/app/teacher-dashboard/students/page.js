...
export default function TeacherStudentsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: studentsRes } = await supabase.from("students").select("class").eq("branch", user.branch);

      const c = {};
      (studentsRes || []).forEach((s) => { c[s.class] = (c[s.class] || 0) + 1; });
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-8">{branch} Section — select a class to view students.</p>
          ...
          {classes.map((c) => (
            <Link key={c.id} href={"/teacher-dashboard/students/" + encodeURIComponent(c.name)} className="group relative border border-slate-200 rounded-2xl p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 block bg-white overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-slate-500">student{(counts[c.name] || 0) === 1 ? "" : "s"} enrolled</p>
                <p className="text-xs text-brand-blue-strong font-medium mt-4 inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform">View class →</p>
              </div>
            </Link>
          ))}
        </div>
